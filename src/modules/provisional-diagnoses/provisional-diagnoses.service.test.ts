...
import { ProvisionalDiagnosesController } from "./provisional-diagnoses.controller";
...
jest.mock("./provisional-diagnoses.repository");

describe("ProvisionalDiagnosesService", () => {
  ...
  beforeEach(() => {
    ...
    (ProvisionalDiagnosesRepository as unknown as jest.Mock).mockImplementation(
      () => ({ findByPatientId }),
    );
  });
  ...
  it("forwards visitId filter when provided", async () => {
    const service = new ProvisionalDiagnosesService(context);
    await service.listProvisionalDiagnosesByPatientId({
      patientId: "patient-1",
      visitId: "visit-9",
      page: 1,
      pageSize: 30,
...
describe("ProvisionalDiagnosesController", () => {
  it("rejects requests without a facility context as unauthorized", async () => {
    const controller = new ProvisionalDiagnosesController();
    const req = { query: { patientId: "p-1" } } as any;
    const res = {} as any;
    const next = jest.fn();

    await controller.getProvisionalDiagnoses(req, res, next);
